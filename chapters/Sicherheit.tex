...
Beim Thema Randomisierung ist wichtig zu erwähnen, dass in einer realen Anwendung die Koeffizienten $m_i$ für $i > 0$ echt zufällig und entsprechend der Gleichverteilung aus $\mathbb{F}_p$ gewählt werden müssen (vgl. Zeile 2 in \autoref{code:create_shares}) \autocite[S. 66]{bsi-richtlinie-2023}.
...
\begin{enumerate}
	...
	\item Shares: Wie im letzten Abschnitt bereits durch den BSI erwähnt, gilt es die Shares geheim zu halten. Um sicherzugehen, dass eine Nachricht nach der Übertragung von dem erwarteten Sender stammt und diese nicht auf dem Weg manipuliert wurde, können beispielsweise Message Authentication Codes (MAC) oder Signaturverfahren eingesetzt werden. 
\end{enumerate}
